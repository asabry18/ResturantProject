...
export default function Reservation(){
    ...
    function validateForm(){
        ...
        if (!formData.phone) newErrors.phone = "Enter a valid phone number (xxx-xxxx-xxxx).";
        if (!formData.totalPerson || formData.totalPerson <= 0) newErrors.totalPerson = "Enter a valid number of people.";
        setErrors(newErrors);
    ...
    return(
        ...
        <div className="reservation">
            ...
            <div className="bookingTitle text-center pt-5 pb-3">
                ...
                <h6 className="opacity-75 lead">We consider all the drivers of change gives you the components you need to change to create a truly happens.</h6>
            </div>
                ...
                <Container className="container w-50 py-5">
                    <form className="bookingForm row row-gap-4 bg-white" onSubmit={handleSubmit}>
                        ...
                        <Col md={6} className="d-flex flex-column">
                            ...
                            <input name="phone" type="tel" placeholder="xxx-xxxx-xxxx" aria-label="Enter your phone number" value={formData.phone} onChange={handleChange} />
                            {errors.phone && <small className="text-danger">{errors.phone}</small>}
                        </Col>
                        <Col md={12} className="d-flex flex-column">
                            <label htmlFor="totalPerson">Total Person</label>
                            <input name="totalPerson" type="number" value={formData.totalPerson} onChange={handleChange}/>
                            {errors.totalPerson && <small className="text-danger">{errors.totalPerson}</small>}
                        </Col>
                        <Col md={12} className="d-flex flex-column">
                            <Button type="submit" className="bookingBtn">Book A Table</Button>
                        </Col>
                    </form>
                </Container>
            </section>
